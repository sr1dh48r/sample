...
static int wln_ioremap(struct uccp_priv *);
static void wln_iounmap(struct uccp_priv *);

static int
wln_ioremap(struct uccp_priv *priv)
{
	/* map wifi_T0 memory */
	priv->wifiT0_mem_addr = (unsigned long)ioremap(WLN_WIFI_T0_BASE,
						       WLN_WIFI_T0_LEN);
	if (priv->wifiT0_mem_addr == 0) {
		pr_err("ioremap() failed for wifi_T0 mem region.\n");
		return -ENOMEM;
	}

	/* map wifi_T1 memory */
	priv->wifiT1_mem_addr = (unsigned long)ioremap(WLN_WIFI_T1_BASE,
						       WLN_WIFI_T1_LEN);
	if (priv->wifiT1_mem_addr == 0) {
		pr_err("ioremap() failed for wifi_T1 mem region.\n");
		iounmap((void __iomem *)priv->wifiT0_mem_addr);
		return -ENOMEM;
	}
	img_dbg_info("wifiT0_mem_addr=0x%X wifiT1_mem_addr=0x%X\n",
			(unsigned int)priv->wifiT0_mem_addr,
			(unsigned int)priv->wifiT1_mem_addr);

	return 0;
}

static void
wln_iounmap(struct uccp_priv *priv)
{
	/* unmap wifi_T0 memory */
	iounmap((void __iomem *)priv->wifiT0_mem_addr);

	/* unmap wifi_T1 memory */
	iounmap((void __iomem *)priv->wifiT1_mem_addr);
}
